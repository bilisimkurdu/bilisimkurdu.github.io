...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">Site Haritası</xsl:when>
						<xsl:otherwise>Site Haritası İndeksi</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">Site Haritası</xsl:with-param>
	...
	<a href="https://bilisimkurdu.github.io"><span>Ev</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<span>Site Haritası İndeksi</span>
	...
	<a href="https://bilisimkurdu.github.io"><span>Ev</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="https://bilisimkurdu.github.io/sitemap.xml"><span>Site Haritası İndeksi</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<p>Geliştiren: <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, bu sayfa, Google veya Bing gibi arama motorları tarafından kullanılması amaçlanan bir XML Site Haritasıdır.</p>
		<p>
			XML site haritaları hakkında daha fazla bilgiyi <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a> adresinde bulabilirsiniz.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Bu site haritası içeriyor						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								URL							</xsl:when>
							<xsl:otherwise>
								URL'ler							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						Bu site haritası index içeriyor						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								site haritası							</xsl:when>
							<xsl:otherwise>
								site haritaları							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				ve 7 Ekim 2024 tarihinde saat 20:38 itibariyle oluşturuldu			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Düşük</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Orta</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">Yüksek</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="https://bilisimkurdu.github.io"><span>Ev</span></a>
		...
		<h2 class="empty-sitemap__title">
			Hay aksi!			<br />
			Burada hiçbir gönderi yok		</h2>
		<div class="empty-sitemap__buttons">
			<a href="https://bilisimkurdu.github.io" class="button">Ana Sayfaya Geri Dön</a>
							<a href="https://bilisimkurdu.github.io/wp-admin/admin.php?page=aioseo-sitemaps" class="button">Site Haritasını Yapılandır</a>
					</div>

					<div class="aioseo-alert yellow">
				Bunu görmeyi beklemiyor muydunuz? Site haritanızın etkinleştirilmiş ve içeriğinizin indekslenecek şekilde ayarlanmış olduğundan emin olun. <a target="_blank" href="https://aioseo.com/docs/how-to-fix-a-404-error-when-viewing-your-sitemap/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap&#038;utm_content=learn-more">Daha Fazlasını Öğrenin →</a>			</div>
			</div>
	<style>
		.hand-magnifier{
